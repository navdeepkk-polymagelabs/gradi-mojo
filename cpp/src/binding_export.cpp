...
int run_binding_external_to_plot(char* c, int length, char** c_return, int* length_return) {
    // Deserialize input_string to json
    std::string input_string = std::string(c, length);
    json::jobject input_json = json::jobject::parse(input_string);
    ...
    try {
        output = gradient_descent_to_plot_cpp(input);
    ...
    for(int i = 0, e = output.all_X.size(); i < e; ++i){
      for(auto iter = output.all_X[i].data(), e = output.all_X[i].data() + output.all_X[i].size(); iter < e; iter++){
        vector_X.push_back(*iter);
      }
    }
    output_json["all_X"] = vector_X;
