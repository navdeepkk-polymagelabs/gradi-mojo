...
libc = CDLL("/home1/navdeep/work/projects/gradi-mojo/cpp/build/lib/gradient_descent.so")
...
run_binding_external_to_plot = libc.run_binding_external_to_plot
run_binding_external_to_plot.argtypes = [c_char_p, c_int, POINTER(c_char_p), POINTER(c_int)]
run_binding_external_to_plot.restype = c_int
...
def run_binding_to_plot(input_string):
    s_return = c_char_p()
    length_return = c_int()
    code = run_binding_external_to_plot(input_string, len(input_string), pointer(s_return), pointer(length_return))
    if code == 0:
        result = string_at(s_return, length_return)
        delete_c_return(s_return)
        return result
...
def gradient_descent_cpp_to_plot(X, D, learning_rate, num_iterations):
    # Serialize input
    # Flatten input matrices to 1D arrays
    input_json = json.dumps(
        {
    ...
    # call python-cpp binding
    result = run_binding_to_plot(input_json.encode('utf-8'))

    # Deserialize output
    output_json = json.loads(result.decode('utf-8'))
    X_out = output_json["all_X"]
    X_out = np.transpose(np.reshape(np.array(X_out), (num_iterations, X.shape[1], X.shape[0])), (0, 2, 1))
